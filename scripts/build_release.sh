...
cd ..
cp -R logtalk/manuals man2308
tar -czf man2308.tgz man2308
mv logtalk lgt2308
tar -cjf lgt2308.tar.bz2 lgt2308
...
cd $dir
dpkg-deb --build debian logtalk_2.30.8-1_all.deb

md5="`md5 -q lgt2308.tar.bz2`"
sudo mkdir -p /opt/local/var/macports/distfiles/logtalk
sudo cp -f lgt2308.tar.bz2 /opt/local/var/macports/distfiles/logtalk/lgt2308.tar.bz2
cd /opt/local/var/macports/sources/rsync.macports.org/release/ports/lang/logtalk/
sudo cp -f Portfile Portfile.old
sudo sed -e 's/^version.*/version 2.30.8/' -i '' Portfile
sudo sed -e "s/^checksums.*/checksums md5 $md5/" -i '' Portfile
sudo sed -e 's/^distname.*/distname lgt2308/' -i '' Portfile
...
cp -R work/logtalk-2.30.8.pkg $dir
...
cd man2308/userman
./userman.sh
mv userman.pdf ../../manpdf2308
cd ../refman
./refman.sh
mv refman.pdf ../../manpdf2308
cd ../..
tar -czf manpdf2308.tgz manpdf2308
