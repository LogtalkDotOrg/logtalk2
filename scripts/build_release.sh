...
cd ..
cp -R logtalk/manuals man2312
tar -czf man2312.tgz man2312
mv logtalk lgt2312
tar -cjf lgt2312.tar.bz2 lgt2312
...
cd $dir
dpkg-deb --build debian logtalk_2.31.2-1_all.deb

md5="`md5 -q lgt2312.tar.bz2`"
sudo mkdir -p /opt/local/var/macports/distfiles/logtalk
sudo cp -f lgt2312.tar.bz2 /opt/local/var/macports/distfiles/logtalk/lgt2312.tar.bz2
cd /opt/local/var/macports/sources/rsync.macports.org/release/ports/lang/logtalk/
sudo cp -f Portfile Portfile.old
sudo sed -e 's/^version.*/version 2.31.2/' -i '' Portfile
sudo sed -e "s/^checksums.*/checksums md5 $md5/" -i '' Portfile
sudo sed -e 's/^distname.*/distname lgt2312/' -i '' Portfile
...
cp -R work/logtalk-2.31.2.pkg $dir
...
cd man2312/userman
./userman.sh
mv userman.pdf ../../manpdf2312
cd ../refman
./refman.sh
mv refman.pdf ../../manpdf2312
cd ../..
tar -czf manpdf2312.tgz manpdf2312
