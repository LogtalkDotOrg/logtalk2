...
cd ..
cp -R logtalk/manuals man2306
tar -czf man2306.tgz man2306
mv logtalk lgt2306
tar -cjf lgt2306.tar.bz2 lgt2306
...
cd $dir
dpkg-deb --build debian logtalk_2.30.6-1_all.deb

md5="`md5 -q lgt2306.tar.bz2`"
sudo mkdir -p /opt/local/var/macports/distfiles/logtalk
sudo cp -f lgt2306.tar.bz2 /opt/local/var/macports/distfiles/logtalk/lgt2306.tar.bz2
cd /opt/local/var/macports/sources/rsync.macports.org/release/ports/lang/logtalk/
sudo cp -f Portfile Portfile.old
sudo sed -e 's/^version.*/version 2.30.6/' -i '' Portfile
sudo sed -e "s/^checksums.*/checksums md5 $md5/" -i '' Portfile
sudo sed -e 's/^distname.*/distname lgt2306/' -i '' Portfile
...
cp -R work/logtalk-2.30.6.pkg $dir
...
cd man2306/userman
./userman.sh
mv userman.pdf ../../manpdf2306
cd ../refman
./refman.sh
mv refman.pdf ../../manpdf2306
cd ../..
tar -czf manpdf2306.tgz manpdf2306
