...
cd ..
cp -R logtalk/manuals man2352
tar -czf man2352.tgz man2352
mv logtalk lgt2352
...
cd $dir
dpkg-deb --build debian logtalk_2.35.2-1_all.deb

md5="`md5 -q lgt2352.tar.bz2`"
sudo mkdir -p /opt/local/var/macports/distfiles/logtalk
sudo cp -f lgt2352.tar.bz2 /opt/local/var/macports/distfiles/logtalk/lgt2352.tar.bz2
cd /opt/local/var/macports/sources/rsync.macports.org/release/ports/lang/logtalk/
sudo mv -f Portfile Portfile.old
sudo cp $dir/lgt2352/scripts/macosx/Portfile .
sudo sed -e 's/^version.*/version 2.35.2/' -i '' Portfile
...
cp -R work/logtalk-2.35.2.pkg $dir
...
cd man2352/userman
./userman.sh
mv userman.pdf ../../manpdf2352
cd ../refman
./refman.sh
mv refman.pdf ../../manpdf2352
cd ../..
tar -czf manpdf2352.tgz manpdf2352
