...
cd ..
cp -R logtalk/manuals man2361
tar -czf man2361.tgz man2361
mv logtalk lgt2361
tar -cjf lgt2361.tar.bz2 lgt2361
...
cd $dir
dpkg-deb --build debian logtalk_2.36.1-1_all.deb

md5="`md5 -q lgt2361.tar.bz2`"
sudo mkdir -p /opt/local/var/macports/distfiles/logtalk
sudo cp -f lgt2361.tar.bz2 /opt/local/var/macports/distfiles/logtalk/lgt2361.tar.bz2
cd /opt/local/var/macports/sources/rsync.macports.org/release/ports/lang/logtalk/
sudo mv -f Portfile Portfile.old
sudo cp $dir/lgt2361/scripts/macosx/Portfile .
sudo sed -e 's/^version.*/version 2.36.1/' -i '' Portfile
...
cp -R work/logtalk-2.36.1.pkg $dir
...
cd man2361/userman
./userman.sh
mv userman.pdf ../../manpdf2361
cd ../refman
./refman.sh
mv refman.pdf ../../manpdf2361
cd ../..
tar -czf manpdf2361.tgz manpdf2361
