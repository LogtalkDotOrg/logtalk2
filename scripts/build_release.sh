...
cd ..
cp -R logtalk/manuals man2360
tar -czf man2360.tgz man2360
mv logtalk lgt2360
tar -cjf lgt2360.tar.bz2 lgt2360
...
cd $dir
dpkg-deb --build debian logtalk_2.36.0-1_all.deb

md5="`md5 -q lgt2360.tar.bz2`"
sudo mkdir -p /opt/local/var/macports/distfiles/logtalk
sudo cp -f lgt2360.tar.bz2 /opt/local/var/macports/distfiles/logtalk/lgt2360.tar.bz2
cd /opt/local/var/macports/sources/rsync.macports.org/release/ports/lang/logtalk/
sudo mv -f Portfile Portfile.old
sudo cp $dir/lgt2360/scripts/macosx/Portfile .
sudo sed -e 's/^version.*/version 2.36.0/' -i '' Portfile
...
cp -R work/logtalk-2.36.0.pkg $dir
...
cd man2360/userman
./userman.sh
mv userman.pdf ../../manpdf2360
cd ../refman
./refman.sh
mv refman.pdf ../../manpdf2360
cd ../..
tar -czf manpdf2360.tgz manpdf2360
