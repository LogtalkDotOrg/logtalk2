...
cd ..
cp -R logtalk/manuals man2350
tar -czf man2350.tgz man2350
mv logtalk lgt2350
tar -cjf lgt2350.tar.bz2 lgt2350
...
cd $dir
dpkg-deb --build debian logtalk_2.35.0-1_all.deb

md5="`md5 -q lgt2350.tar.bz2`"
sudo mkdir -p /opt/local/var/macports/distfiles/logtalk
sudo cp -f lgt2350.tar.bz2 /opt/local/var/macports/distfiles/logtalk/lgt2350.tar.bz2
cd /opt/local/var/macports/sources/rsync.macports.org/release/ports/lang/logtalk/
sudo cp -f Portfile Portfile.old
sudo sed -e 's/^version.*/version 2.35.0/' -i '' Portfile
...
cp -R work/logtalk-2.35.0.pkg $dir
...
cd man2350/userman
./userman.sh
mv userman.pdf ../../manpdf2350
cd ../refman
./refman.sh
mv refman.pdf ../../manpdf2350
cd ../..
tar -czf manpdf2350.tgz manpdf2350
