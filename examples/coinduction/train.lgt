...
%		:- initialization(init_clocks).
		...
		init_clocks :-
			init_stack(wall_clock, 0),
			init_stack(train_clock, 0),
     		init_stack(gate_clock, 0),
			init_stack(controller_clock, 0).

		:- public(driver/9).
		:- coinductive1(driver/9).
	
		driver(S0, S1, S2, T, T0, T1, T2, [X| Rest], [(X,T)| R]) :-
%			get_stack(wall_clock, T),
%			get_stack(train_clock, T0),
%			get_stack(controller_clock, T1),
%			get_stack(gate_clock, T2),
			train(S0, X, S00, T, T0, T00),
			gate(S1, X, S10, T, T1, T10),
			controller(S2, X, S20, T, T2, T20),
			clpr:{TA > T},% write('**** NEXT ****'), nl,
%			set_stack(wall_clock, TA),
%			set_stack(train_clock, T00),
%			set_stack(controller_clock, T10),
%    		set_stack(gate_clock, T20),
			driver(S00, S10, S20, TA, T00, T10, T20, Rest, R).
