...
:- category(volumes2d).
    ...
    interval_volume(_, _, _, C, D , 0, _, Acc, Area) :-
		Area is (D-C)*Acc,
		!.
    interval_volume(Function, A, B, Down, Up, N, NP, Acc, Area) :-
		c(NP, N, C),
		w(NP, N, W),
		YK is Down + (Up-Down)*C,
		interval_area(Function, A, B, YK, NP, NP, 0.0, Area1d), !,	% cut needed due to SWI-Prolog weird bug
		N2 is N - 1,
		Acc2 is Acc + W*Area1d,
		interval_volume(Function, A, B, Down, Up, N2, NP, Acc2, Area).
	...
	trapezium_volume(Function, A, B, C, D, Area) :-
		...
		Area is 0.25*(B-A)*(D-C)*(F1+F2+F3+F4).
