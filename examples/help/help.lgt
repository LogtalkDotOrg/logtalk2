...
:- object(help).
	...
	PredicateFunctor/Arity :-
		(	built_in_method(PredicateFunctor, Arity, File) ->
			true
		;	built_in_predicate(PredicateFunctor, Arity, File) ->
			true
		;	control(PredicateFunctor, Arity, File) ->
			true
		),
		open(File).
	...
	NonTerminalFunctor//Arity :-
		(	built_in_non_terminal(NonTerminalFunctor, Arity, File) ->
			open(File)
		;	fail
		).
