...
'$lgt_split_metafile'(Source) :-
	...
	('$lgt_compiler_option'(report, on) ->
		...
		nl, write('>>> source metafile splited'), nl
		;
		true).



% '$lgt_needs_respliting'(+atom)
%
% source metafile needs respliting; assume that the user does not 
% temper with the files generated by the spliting of the metafile

'$lgt_needs_respliting'(Source) :-
	'$lgt_file_name'(logtalk, Source, Loader),
	(\+ '$lgt_file_exists'(Loader) ->
		true
		;
		'$lgt_file_name'(metafile, Source, Metafile),
		('$lgt_compare_file_mtimes'(Result, Metafile, Loader) ->
			Result = (>)
			;
			true)).
...
'$lgt_create_loader_file'(Source, Cache, Entities) :-	
	'$lgt_file_name'(logtalk, Source, Loader),
		...
		(open(Loader, write, Stream),
		 ...
		 '$lgt_create_loader_file_aux'(Entities2, Stream),
		 write_term(Stream, '])).', []), nl(Stream), nl(Stream),
...
'$lgt_create_loader_file_aux'([Entity1, Entity2| Entities], Stream) :-
	write_term(Stream, '        ', []),
	write_canonical(Stream, Entity1),
	write_term(Stream, ',', []), nl(Stream),
	'$lgt_create_loader_file_aux'([Entity2| Entities], Stream).
...
'$lgt_copy_metafile_term'(Input, Term, CacheAcc, Cache, EntityAcc, Entities) :-
	'$lgt_opening_entity_directive'(Term, Entity),
	!,
	('$lgt_compiler_option'(report, on) ->
...
'$lgt_copy_metafile_entity_terms'(Input, Output) :-
	...
	(Term = end_of_file ->
		...
		write_canonical(Output, Term),
		write_term(Output, '.', []), nl(Output),
		('$lgt_closing_entity_directive'(Term) ->
			close(Output)
			;
			'$lgt_copy_metafile_entity_terms'(Input, Output))).
...
'$lgt_copy_cached_metafile_terms'([Term| Terms], Output) :-
	write_canonical(Output, Term),
	write_term(Output, '.', []), nl(Output),
	'$lgt_copy_cached_metafile_terms'(Terms, Output).
