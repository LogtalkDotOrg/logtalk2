...
category_property(Ctg, Prop) :-				% static/dynamic property
	'$lgt_current_category_'(Ctg, _, _, _, Prop, _).

category_property(Ctg, synchronized) :-
	'$lgt_current_category_'(Ctg, _, _, _, _, yes).
...
abolish_category(Ctg) :-
	(	'$lgt_current_category_'(Ctg, Prefix, _, _, Type, _) ->
		(	Type == (dynamic) ->
...
'$lgt_alias_pred'(Ctg1, _, Alias, Pred, _) :-
	...
	'$lgt_current_category_'(Ctg2, Prefix, _, _, _, _),
...
'$lgt_pp_rclause'('$lgt_current_category_'(Ctg, Prefix, Dcl, Def, Mode, Synchronized)) :-
	'$lgt_pp_category_'(Ctg, Prefix, Dcl, Def, _, Mode),
	(	'$lgt_pp_synchronized_' ->
		Synchronized = yes
	;	Synchronized = no
	).
...
'$lgt_mt_threaded_call'(Goal, Queue) :-
	thread_self(Id),
	call(Goal),
	thread_send_message(Queue, '$lgt_result'(Id, Goal)).
...
'$lgt_mt_threaded_exit_handler'(Id, Queue) :-
	...
	catch(thread_send_message(Queue, '$lgt_status'(Id, Status)), _, thread_detach(Id)).
...
'$lgt_mt_threaded_and_exit'(Ids, Results) :-
	thread_get_message('$lgt_status'(Id, Result)),
...
'$lgt_mt_threaded_and_exit'(exception(Error), Id, Ids, Results) :-
	...
	(	Error == '$lgt_terminated' ->
		'$lgt_mt_threaded_and_exit'(Ids, Results)
	;	'$lgt_mt_threaded_call_cancel'(Ids, Results, Id),
		throw(Error)
	).
...
'$lgt_mt_threaded_or_exit'(true, Id, Ids, Results) :-
	thread_get_message('$lgt_result'(Id, TGoal)),
	'$lgt_mt_threaded_record_result'(Results, Id, true),
	'$lgt_mt_threaded_call_cancel'(Ids, Results, Id),
...
'$lgt_mt_threaded_or_record_failure'([id(Id, _, fail)| Results], Id, Continue) :-
	!,
	(	var(Continue) ->	% we still don't know if there are any pending results
...
'$lgt_mt_threaded_or_continue'([id(_, _, Done)| Results], Continue) :-
	(	var(Done) ->
		% we found a thread whose result is still pending
		Continue = true
	;	% otherwise continue looking for a thread with a still pending result
...
% '$lgt_mt_threaded_call_cancel'(+list(thread_identifier), +list, +thread_identifier)
%
...
'$lgt_mt_threaded_call_cancel'(Ids, Results, ProtectedId) :-
