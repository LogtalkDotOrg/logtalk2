		...
		('$lgt_current_object_'(Self, _, Dcl, Def, _, _),
		 ('$lgt_call'(Dcl, Pred, Scope, _, _, SCtn, _) ->
			((Scope = p(_); This = SCtn) ->
				...
				asserta('$lgt_self_lookup_cache_'(Self, GPred, GSender, GThis, GSelf, Call)),
				GPred = Pred, GSender = This, GThis = Self, GSelf =Self,
				call(Call)
				;
		...
		('$lgt_current_object_'(Obj, _, Dcl, Def, _, _) ->
			('$lgt_call'(Dcl, Pred, Scope, _, _, _, _) ->
				(Scope = p(p(_)) ->
					...
					GPred = Pred, GSender = Sender, GSelf = Obj, GThis = Obj,
					\+ ('$lgt_before_'(Obj, Pred, Sender, _, BCall), \+ call(BCall)),
					call(Call),
					\+ ('$lgt_after_'(Obj, Pred, Sender, _, ACall), \+ call(ACall))
					;
...
'$lgt_report_unknown_objects' :-
	findall(
		Obj,
		('$lgt_pp_referenced_object_'(Obj), \+ ('$lgt_current_object_'(Obj, _, _, _, _, _); '$lgt_pp_entity_'(_, Obj, _, _))),
		Objs),
	(Objs \= [] ->
...
'$lgt_report_unknown_protocols' :-
	findall(
		Ptc,
		('$lgt_pp_referenced_protocol_'(Ptc), \+ ('$lgt_current_protocol_'(Ptc, _, _); '$lgt_pp_entity_'(_, Ptc, _, _))),
		Ptcs),
	(Ptcs \= [] ->
...
'$lgt_report_unknown_categories' :-
	findall(
		Ctg,
		('$lgt_pp_referenced_category_'(Ctg), \+ ('$lgt_current_category_'(Ctg, _, _); '$lgt_pp_entity_'(_, Ctg, _, _))),
		Ctgs),
	(Ctgs \= [] ->
