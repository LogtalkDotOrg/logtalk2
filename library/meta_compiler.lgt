...
:- object(meta_compiler,
	...
	goal_expansion(meta::include(Closure, List, Included), ExpandedGoal) :-
		decompose_closure(Closure, 1, Functor, Arity, Args),
		...
		;	extend_closure(Functor, Args, [Head], Goal),
			Clauses0 = [
				include_([], _, []),
				(include_([Head| Tail], Args, Result) :-
					(	Goal ->
						Result = [Head| Rest]
					;	Result = Rest
					),
					include_(Tail, Args, Rest))
				],
	...
	goal_expansion(meta::exclude(Closure, List, Excluded), ExpandedGoal) :-
		...
		;	extend_closure(Functor, Args, [Head], Goal),
			Clauses0 = [
				exclude_([], _, []),
				(exclude_([Head| Tail], Args, Result) :-
					(	Goal ->
						Result = Rest
					;	Result = [Head| Rest]
					),
					exclude_(Tail, Args, Rest))
				],
	...
	goal_expansion(meta::partition(Closure, List, Included, Excluded), ExpandedGoal) :-
		...
		;	extend_closure(Functor, Args, [Head], Goal),
			Clauses0 = [
				partition_([], _, [], []),
				(partition_([Head| Tail], Args, RIncluded, RExcluded) :-
					(   Goal ->
					...
					partition_(Tail, Args, RestIncluded, RestExcluded))
				],
	...
	goal_expansion(meta::partition(Closure, List, Value, Less, Equal, Greater), ExpandedGoal) :-
		decompose_closure(Closure, 3, Functor, Arity, Args),
		...
		;	extend_closure(Functor, Args, [Order, X, Y], Goal),
			Clauses0 = [
				partition_([], _, _, [], [], []),
				(partition_([X| Xs], Y, Args, RLess, REqual, RGreater) :-
					Goal,
					partition_(Order, X, Xs, Y, Args, RLess, REqual, RGreater)),		
				(partition_(<, X, Xs, Y, Args, [X| RLess], REqual, RGreater) :-
					partition_(Xs, Y, Args, RLess, REqual, RGreater)),
				(partition_(=, X, Xs, Y, Args, RLess, [X| REqual], RGreater) :-
					partition_(Xs, Y, Args, RLess, REqual, RGreater)),
				(partition_(>, X, Xs, Y, Args, RLess, REqual, [X| RGreater]) :-
					partition_(Xs, Y, Args, RLess, REqual, RGreater))
				],
	...
	goal_expansion(meta::map(Closure, List1, List2), ExpandedGoal) :-
		decompose_closure(Closure, 2, Functor, Arity, Args),
		aux_predicate_functor(map, 3, Functor, Arity, AuxFunctor),
		(	generated_predicate(AuxFunctor/2) ->
			replace_functor([map_(List1, List2, Args)], map_, AuxFunctor, [ExpandedGoal])
		;	extend_closure(Functor, Args, [Head1, Head2], Goal),
			Clauses0 = [
					map_([], [], _),
					(map_([Head1| Tail1], [Head2| Tail2], Args) :-
						Goal, map_(Tail1, Tail2, Args))
				],
			replace_functor([map_(List1, List2, Args)| Clauses0], map_, AuxFunctor, [ExpandedGoal| Clauses]),
			logtalk::compile_clauses(Clauses),
			assertz(generated_predicate(AuxFunctor/2))
		).
	...
	goal_expansion(meta::map(Closure, List1, List2, List3), ExpandedGoal) :-
		...
		(	generated_predicate(AuxFunctor/3) ->
			replace_functor([map_(List1, List2, List3, Args)], map_, AuxFunctor, [ExpandedGoal])
		;	extend_closure(Functor, Args, [Head1, Head2, Head3], Goal),
			Clauses0 = [
					map_([], [], [], _),
					(map_([Head1| Tail1], [Head2| Tail2], [Head3| Tail3], Args) :-
						Goal, map_(Tail1, Tail2, Tail3, Args))
				],
			replace_functor([map_(List1, List2, List3, Args)| Clauses0], map_, AuxFunctor, [ExpandedGoal| Clauses]),
			logtalk::compile_clauses(Clauses),
			assertz(generated_predicate(AuxFunctor/3))
		).

	goal_expansion(meta::map(Closure, List1, List2, List3, List4), ExpandedGoal) :-
		decompose_closure(Closure, 4, Functor, Arity, Args),
		aux_predicate_functor(map, 5, Functor, Arity, AuxFunctor),
		(	generated_predicate(AuxFunctor/4) ->
			replace_functor([map_(List1, List2, List3, List4, Args)], map_, AuxFunctor, [ExpandedGoal])
		;	extend_closure(Functor, Args, [Head1, Head2, Head3, Head4], Goal),
			Clauses0 = [
					map_([], [], [], [], _),
					(map_([Head1| Tail1], [Head2| Tail2], [Head3| Tail3], [Head4| Tail4], Args) :-
						Goal, map_(Tail1, Tail2, Tail3, Tail4, Args))
				],
			replace_functor([map_(List1, List2, List3, List4, Args)| Clauses0], map_, AuxFunctor, [ExpandedGoal| Clauses]),
			logtalk::compile_clauses(Clauses),
			assertz(generated_predicate(AuxFunctor/4))
		).

	goal_expansion(meta::map(Closure, List1, List2, List3, List4, List5), ExpandedGoal) :-
		decompose_closure(Closure, 5, Functor, Arity, Args),
		aux_predicate_functor(map, 6, Functor, Arity, AuxFunctor),
		(	generated_predicate(AuxFunctor/5) ->
			replace_functor([map_(List1, List2, List3, List4, List5, Args)], map_, AuxFunctor, [ExpandedGoal])
		;	extend_closure(Functor, Args, [Head1, Head2, Head3, Head4, Head5], Goal),
			Clauses0 = [
					map_([], [], [], [], [], _),
					(map_([Head1| Tail1], [Head2| Tail2], [Head3| Tail3], [Head4| Tail4], [Head5| Tail5], Args) :-
						Goal, map_(Tail1, Tail2, Tail3, Tail4, Tail5, Args))
				],
			replace_functor([map_(List1, List2, List3, List4, List5, Args)| Clauses0], map_, AuxFunctor, [ExpandedGoal| Clauses]),
			logtalk::compile_clauses(Clauses),
			assertz(generated_predicate(AuxFunctor/5))
		).

	goal_expansion(meta::map(Closure, List1, List2, List3, List4, List5, List6), ExpandedGoal) :-
		decompose_closure(Closure, 6, Functor, Arity, Args),
		aux_predicate_functor(map, 7, Functor, Arity, AuxFunctor),
		(	generated_predicate(AuxFunctor/6) ->
			replace_functor([map_(List1, List2, List3, List4, List5, List6, Args)], map_, AuxFunctor, [ExpandedGoal])
		;	extend_closure(Functor, Args, [Head1, Head2, Head3, Head4, Head5, Head6], Goal),
			Clauses0 = [
					map_([], [], [], [], [], [], _),
					(map_([Head1| Tail1], [Head2| Tail2], [Head3| Tail3], [Head4| Tail4], [Head5| Tail5], [Head6| Tail6], Args) :-
						Goal, map_(Tail1, Tail2, Tail3, Tail4, Tail5, Tail6, Args))
				],
			replace_functor([map_(List1, List2, List3, List4, List5, List6, Args)| Clauses0], map_, AuxFunctor, [ExpandedGoal| Clauses]),
			logtalk::compile_clauses(Clauses),
	...
	goal_expansion(meta::map(Closure, List1, List2, List3, List4, List5, List6, List7), ExpandedGoal) :-
		decompose_closure(Closure, 7, Functor, Arity, Args),
		aux_predicate_functor(map, 8, Functor, Arity, AuxFunctor),
		(	generated_predicate(AuxFunctor/7) ->
			replace_functor([map_(List1, List2, List3, List4, List5, List6, List7, Args)], map_, AuxFunctor, [ExpandedGoal])
		;	extend_closure(Functor, Args, [Head1, Head2, Head3, Head4, Head5, Head6, Head7], Goal),
			Clauses0 = [
					map_([], [], [], [], [], [], [], _),
					(map_([Head1| Tail1], [Head2| Tail2], [Head3| Tail3], [Head4| Tail4], [Head5| Tail5], [Head6| Tail6], [Head7| Tail7], Args) :-
						Goal, map_(Tail1, Tail2, Tail3, Tail4, Tail5, Tail6, Tail7, Args))
				],
			replace_functor([map_(List1, List2, List3, List4, List5, List6, List7, Args)| Clauses0], map_, AuxFunctor, [ExpandedGoal| Clauses]),
			logtalk::compile_clauses(Clauses),
	...
	goal_expansion(meta::fold_left(Closure, AccE, ListE, ResultE), ExpandedGoal) :-
		...
		;	extend_closure(Functor, Args, [Acc, Head, Acc2], Goal),
			Clauses0 = [
					...
					(fold_left_([Head| Tail], Args, Acc, Result) :-
						Goal, fold_left_(Tail, Args, Acc2, Result))
				],
	...
	goal_expansion(meta::fold_right(Closure, AccE, ListE, ResultE), ExpandedGoal) :-
		...
		;	extend_closure(Functor, Args, [Head, Acc2, Result], Goal),
			Clauses0 = [
					...
					(fold_right_([Head| Tail], Args, Acc, Result) :-
						fold_right_(Tail, Args, Acc, Acc2), Goal)
				],
	...
	goal_expansion(meta::scan_left(Closure, AccE, ListE, ResultsE), ExpandedGoal) :-
		...
		;	extend_closure(Functor, Args, [Acc, Head, Acc2], Goal),
			Clauses0 = [
					...
					(scan_left_([Head| Tail], Args, Acc, [Acc| Results]) :-
						Goal, scan_left_(Tail, Args, Acc2, Results))
				],
	...
	goal_expansion(meta::scan_right(Closure, AccE, ListE, ResultsE), ExpandedGoal) :-
		...
		;	extend_closure(Functor, Args, [Head, Acc2, Result], Goal),
			Clauses0 = [
					...
					(scan_right_([Head| Tail], Args, Acc, [Result, Acc2| Results]) :-
						scan_right_(Tail, Args, Acc, [Acc2| Results]), Goal)
				],
	...
	goal_expansion(meta::map_reduce(Map, Reduce, AccE, ListE, ResultE), ExpandedGoal) :-
		decompose_closure(Map, 2, MapFunctor, _, MapArgs),
		decompose_closure(Reduce, 3, ReduceFunctor, _, ReduceArgs),
		...
		;	extend_closure(MapFunctor, MapArgs, [Head, Head2], MapGoal),
			...
			Clauses0 = [
					...
					(map_reduce_([Head| Tail], MapArgs, ReduceArgs, Acc, Result) :-
						MapGoal,
						ReduceGoal,
						map_reduce_(Tail, MapArgs, ReduceArgs, Acc2, Result))
				],
	...
	decompose_closure({Free}/Parameters>>Goal, _, Functor, Arity, FreeList) :-
		!,
		callable(Goal),
		gensym('_aux_l_', Functor),
		conjunction_to_list(Free, FreeList, Arity),
		append(FreeList, Parameters, Args),
		Head =.. [Functor| Args],
		logtalk::compile_clauses([(Head :- Goal)]).
	decompose_closure({Free}/Closure, MetaArity, Functor, Arity, FreeList) :-
		!,
		callable(Closure),
		gensym('_aux_l_', Functor),
		conjunction_to_list(Free, FreeList, Arity),
	...
	decompose_closure(Parameters>>Goal, _, Functor, 0, []) :-
		...
		logtalk::compile_clauses([(Head :- Goal)]).
	decompose_closure(Object::Closure, MetaArity, Object::Functor, Arity, Args) :-
		!,
		nonvar(Closure),
		decompose_closure(Closure, MetaArity, Functor, Arity, Args).
	decompose_closure({Closure}, MetaArity, {Functor}, Arity, Args) :-
		!,
		nonvar(Closure),
		decompose_closure(Closure, MetaArity, Functor, Arity, Args).
	decompose_closure(Closure, _, Functor, Arity, Args) :-
		callable(Closure),
